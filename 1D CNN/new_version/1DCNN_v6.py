...
if not os.path.exists(scaler_dir):
    ...
    print(f"Utworzono folder '{scaler_dir}' na modele i skalery")

# Zapisanie skalera
scaler_path = os.path.join(scaler_dir, 'emg_scaler.pkl')
joblib.dump(scaler, scaler_path)
print(f"Skaler został zapisany jako {scaler_path}")
...
# Tworzenie katalogów na wykresy rozkładu cech przed i po normalizacji
before_norm_dir = os.path.join(plots_dir, 'before_normalization')
after_norm_dir = os.path.join(plots_dir, 'after_normalization')

if not os.path.exists(before_norm_dir):
    os.makedirs(before_norm_dir)
    print(f"Utworzono folder '{before_norm_dir}' na wykresy przed normalizacją")

if not os.path.exists(after_norm_dir):
    os.makedirs(after_norm_dir)
    print(f"Utworzono folder '{after_norm_dir}' na wykresy po normalizacji")

# === Wizualizacja rozkładu każdej cechy dla każdego gestu ===
print("\n=== Tworzenie wykresów rozkładu cech dla każdego gestu ===")

# Liczba cech na jeden wykres
features_per_plot = 6

# 1. Wykresy przed normalizacją
for i in range(0, len(feature_cols), features_per_plot):
    # Wybierz podzbiór cech
    subset_features = feature_cols[i:i + features_per_plot]
    num_features = len(subset_features)

    # Określ układ wykresu - mniej więcej kwadratowy
    rows = int(np.ceil(np.sqrt(num_features)))
    cols = int(np.ceil(num_features / rows))

    plt.figure(figsize=(5 * cols, 4 * rows))

    for j, feature in enumerate(subset_features):
        plt.subplot(rows, cols, j + 1)

        # Rysuj rozkład dla każdego gestu
        for gesture in sorted(df_combined['GESTURE'].unique()):
            gesture_data = df_combined[df_combined['GESTURE'] == gesture][feature].values
            if len(gesture_data) > 0:
                sns.kdeplot(gesture_data, label=f'Gest {gesture}', fill=True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(before_norm_dir, f'features_{i + 1}_to_{i + num_features}.png'))
    plt.close()

print(f"Zapisano wykresy rozkładu cech przed normalizacją w folderze '{before_norm_dir}'")

# 2. Wykresy po normalizacji
# Najpierw musimy uzyskać znormalizowane wartości dla każdej cechy i każdego gestu

# Przygotowanie słownika na znormalizowane dane dla każdej cechy i gestu
normalized_features = {}
for feature in feature_cols:
    normalized_features[feature] = {}
    for gesture in sorted(df_combined['GESTURE'].unique()):
        normalized_features[feature][gesture] = []

# Wypełnienie słownika znormalizowanymi danymi
# Używamy danych z meta_test, ponieważ mamy już znormalizowane wartości w X_test_flat
X_test_flat_restructured = X_test_flat.reshape(X_test.shape[0], X_test.shape[1], -1)

for i, meta in enumerate(meta_test):
    gesture = meta[0]
    for window_idx in range(seq_length):  # Dla każdego okna w sekwencji
        for feature_idx, feature_name in enumerate(feature_cols):
            # Dodaj znormalizowaną wartość do odpowiedniego gestu
            normalized_features[feature_name][gesture].append(X_test_flat_restructured[i, window_idx, feature_idx])

# Tworzenie wykresów po normalizacji
for i in range(0, len(feature_cols), features_per_plot):
    # Wybierz podzbiór cech
    subset_features = feature_cols[i:i + features_per_plot]
    num_features = len(subset_features)
    ...
    for j, feature in enumerate(subset_features):
        ...
        # Rysuj rozkład dla każdego gestu
        for gesture in sorted(df_combined['GESTURE'].unique()):
            if len(normalized_features[feature][gesture]) > 10:  # Minimalna liczba punktów dla wiarygodnego rozkładu
                sns.kdeplot(normalized_features[feature][gesture], label=f'Gest {gesture}', fill=True, alpha=0.3)

        plt.title(f'{feature} (znormalizowane)')
        plt.xlabel('Wartość')
        plt.ylabel('Gęstość')
        plt.legend(fontsize='small')
        plt.grid(True, linestyle='--', alpha=0.6)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(after_norm_dir, f'features_{i + 1}_to_{i + num_features}.png'))
    plt.close()

print(f"Zapisano wykresy rozkładu cech po normalizacji w folderze '{after_norm_dir}'")

# === Dodatkowy wykres - Najwyższa separacja gestów dla top-10 cech ===
# Wybierz 10 najważniejszych cech
top_features = [x[0] for x in sorted_features[:10]]

# Dla każdej cechy, znajdź parę gestów z największą separacją
best_separations = []
...
for feature in top_features:
    ...
    for g1 in sorted(df_combined['GESTURE'].unique()):
        for g2 in sorted(df_combined['GESTURE'].unique()):
            if g1 >= g2:  # Unikaj duplikacji i porównywania gestu z samym sobą
                continue

            # Dane dla obu gestów
            g1_data = df_combined[df_combined['GESTURE'] == g1][feature].values
            g2_data = df_combined[df_combined['GESTURE'] == g2][feature].values

            # Oblicz separację (różnicę średnich znormalizowaną przez odchylenie standardowe)
            if len(g1_data) > 0 and len(g2_data) > 0:
                mean_diff = abs(np.mean(g1_data) - np.mean(g2_data))
                std_pooled = np.sqrt((np.var(g1_data) + np.var(g2_data)) / 2)

                if std_pooled > 0:
                    separation = mean_diff / std_pooled
                    if separation > max_separation:
                        max_separation = separation
                        best_pair = (g1, g2)

    if best_pair:
        best_separations.append((feature, best_pair, max_separation))

# Sortuj według separacji
best_separations.sort(key=lambda x: x[2], reverse=True)

# Twórz wykres dla par gestów z największą separacją
plt.figure(figsize=(15, 12))

for i, (feature, (g1, g2), separation) in enumerate(best_separations[:min(10, len(best_separations))]):
    plt.subplot(3, 4, i + 1)

    # Dane dla wybranych gestów
    g1_data = df_combined[df_combined['GESTURE'] == g1][feature].values
    g2_data = df_combined[df_combined['GESTURE'] == g2][feature].values

    # Rysuj rozkłady
    sns.kdeplot(g1_data, label=f'Gest {g1}', fill=True, alpha=0.4)
    sns.kdeplot(g2_data, label=f'Gest {g2}', fill=True, alpha=0.4)

    plt.title(f'{feature}\nSeparacja: {separation:.2f}')
    plt.xlabel('Wartość')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()

plt.tight_layout()
plt.savefig(os.path.join(plots_dir, 'top_feature_separations.png'))
plt.close()

print(f"Zapisano wykres najlepszych separacji cech w '{plots_dir}/top_feature_separations.png'")

# === Wykres macierzy separacji gestów ===
# Tworzenie macierzy pokazującej, jak dobrze rozróżnialne są poszczególne pary gestów
gesture_pairs = []
for g1_idx, g1 in enumerate(sorted(df_combined['GESTURE'].unique())):
    for g2_idx, g2 in enumerate(sorted(df_combined['GESTURE'].unique())):
        if g1_idx >= g2_idx:  # Unikaj duplikacji i porównywania gestu z samym sobą
            continue
        gesture_pairs.append((g1, g2))

# Dla każdej pary gestów, oblicz średnią separację dla top-10 cech
separation_matrix = np.zeros((len(gestures), len(gestures)))
    ...
    for g1_idx, g1 in enumerate(sorted(df_combined['GESTURE'].unique())):
        for g2_idx, g2 in enumerate(sorted(df_combined['GESTURE'].unique())):
            ...
            if len(g1_data) > 0 and len(g2_data) > 0:
                ...
                if std_pooled > 0:
                    separation = mean_diff / std_pooled
                    separation_matrix[g1_idx, g2_idx] += separation / len(top_features)
                    separation_matrix[g2_idx, g1_idx] = separation_matrix[g1_idx, g2_idx]  # Symetryczna

# Wizualizacja macierzy separacji
plt.figure(figsize=(12, 10))
mask = np.triu(np.ones_like(separation_matrix, dtype=bool), k=0)  # Maska dla dolnego trójkąta
sns.heatmap(separation_matrix, annot=True, cmap='YlGnBu',
            xticklabels=[f'Gest {g}' for g in sorted(df_combined['GESTURE'].unique())],
            yticklabels=[f'Gest {g}' for g in sorted(df_combined['GESTURE'].unique())],
            mask=mask)
plt.title('Średnia separacja między gestami\n(dla top-10 cech)')
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, 'gesture_separation_matrix.png'))
plt.close()

print(f"Zapisano macierz separacji gestów w '{plots_dir}/gesture_separation_matrix.png'")
